...
const handleGit = require("./handleGit");
...
function activate(context) {
    ...
    vscode.window.onDidCloseTerminal((t) => {
        terminal_1.TerminalHandler.get().onTerminalClosed(t);
    }, null, context.subscriptions);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('lifeboatapi.shareFile', (file) => {
        return handleGit.shareSelectedFile(context, file);
    }));
    // Add Library
    context.subscriptions.push(vscode.commands.registerCommand('lifeboatapi.cloneGitLibrary', (file) => {
        return handleGit.addLibraryFromURL(context, file);
    }));
    // Remove Library
    context.subscriptions.push(vscode.commands.registerCommand('lifeboatapi.removeLibrary', (file) => {
        return handleGit.removeSelectedLibrary(context, file);
    }));
    // Update Libraries
    context.subscriptions.push(vscode.commands.registerCommand('lifeboatapi.updateLibraries', (file) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("lifeboatapi.updateAllSettings", () => {
        ...
        for (let folder of vscode.workspace.workspaceFolders ?? []) {
            ...
            isSWWorkspace || (isSWWorkspace = isSWProject);
            if (isSWProject) {
        ...
        for (let folder of vscode.workspace.workspaceFolders ?? []) {
            if (utils.isStormworksProject(folder)) {
                ...
                promises.push(Promise.resolve().then(() => {
                    ...
                    let libraries = libConfig.get("gitLibraries");
                    if (libraries === null) {
                        if (!utils.isMicrocontrollerProject(folder)) { // addon
